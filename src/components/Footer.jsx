...
import { Instagram, TikTok, Facebook, Whatsapp, Mail, MapPin, YocoMark, CourierMark, Lock } from './Icon.jsx';

const COLS = [
  {
    h: 'Shop',
    links: [
      { l: 'Boy Scoops',  href: '#builder' },
      { l: 'Girl Scoops', href: '#builder' },
      { l: 'Baby Scoops', href: '#builder' },
      { l: 'Gift cards',  href: '#'        },
    ],
  },
  {
    h: 'Help',
    links: [
      { l: 'Track your order', href: '#'    },
      { l: 'Delivery info',    href: '#faq' },
      { l: 'FAQ',              href: '#faq' },
      { l: 'Contact us',       href: 'mailto:hello@scooped.co.za' },
    ],
  },
  {
    h: 'About',
...
const POLICY = [
  ...
  { l: 'Shipping policy', href: '#' },
];

export default function Footer() {
  return (
    <footer className="border-t border-line bg-soft mt-12">
      <div className="mx-auto max-w-[1180px] px-6 sm:px-7 pt-16 pb-10">
        ...
        <div className="grid grid-cols-2 gap-10 md:grid-cols-4 md:gap-8 mb-12">
          ...
          {COLS.map((col) => (
            <div key={col.h}>
              <div className="mb-4 text-[11px] font-extrabold uppercase tracking-[.16em] opacity-50">{col.h}</div>
              <ul className="space-y-2.5 text-[14px]">
                {col.links.map((l) => (
                  <li key={l.l}>
                    <a href={l.href} className="opacity-75 transition hover:text-coral hover:opacity-100">{l.l}</a>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="mt-8 flex flex-col items-center justify-between gap-4 border-t border-line pt-6 text-[12px] opacity-55 md:flex-row">
          ...
          <ul className="flex flex-wrap items-center gap-x-5 gap-y-1">
            {POLICY.map((p) => (
              <li key={p.l}><a href={p.href} className="hover:text-ink hover:opacity-100">{p.l}</a></li>
            ))}
            <li><Link to="/admin" className="hover:text-ink hover:opacity-100">Admin</Link></li>
          </ul>
        </div>

        <p className="mt-4 text-center text-[11px] opacity-30">
          &ldquo;{BRAND.name}&rdquo; is a placeholder brand name.
        </p>
      </div>
    </footer>
  );
...
function SocialIcon({ href, label, children }) {
  return (
    <a href={href} aria-label={label} target="_blank" rel="noopener noreferrer"
       className="grid h-9 w-9 place-items-center rounded-lg border border-line bg-white text-ink/70 transition hover:-translate-y-0.5 hover:border-coral hover:text-coral hover:shadow-soft">
      {children}
    </a>
  );
}
